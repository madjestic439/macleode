...
class Ihm:
    ...
    def creat_ihm(self):
        ...
        self.main_pan.pack(side=TOP, expand=True, fill=BOTH, pady=2, padx=2)
        self.fr_input = LabelFrame(self.main_pan, relief='groove', borderwidth=5, text='Input')
        self.fr_input_bts = Frame(self.fr_input, borderwidth=0, relief='groove')
        self.bt_select_dir = Button(self.fr_input_bts, text='Add a directory...', command=self.select_dir, width=30)
        ...
        self.fr_input_bts.pack(side='left', fill=BOTH, expand=False)
        self.fr_input_lst.pack(side='right', fill=BOTH, expand=True)
        #self.fr_input.pack(side="top", padx=10, pady=10, fill="x")
        self.fr_input.pack(fill='both', anchor=CENTER)
        self.bt_go = Button(self.main_pan, text='Go...', command=self.go, state=DISABLED)
        ...
        self.fr_output_bts = Frame(self.fr_output, borderwidth=0, relief='groove')
        self.bt_del_file = Button(self.fr_output_bts, text='Delete file', command=self.del_file, state=DISABLED, width=30)
        ...
        self.lbl_nb_file.pack(side="top", fill="x", pady=5)
        self.lbl_nb_crp = Label(self.fr_output_bts, text='')
        self.lbl_nb_crp.pack(side="top", fill="x", pady=5)
        self.lbl_nb_file_del = Label(self.fr_output_bts, text='')
        self.lbl_nb_file_del.pack(side="top", fill="x", pady=5)
        self.fr_output_bts.pack(side='left', fill=BOTH, expand=False)
        self.fr_output_lst = Frame(self.fr_output, borderwidth=2, relief='groove')
        self.list_crp = Listbox(self.fr_output_lst, width=150, height=20)
        self.list_crp.pack(side="top", fill=BOTH, expand=True)
        self.fr_output_lst.pack(side='right', fill=BOTH, expand=True)
        self.fr_output.pack(fill=BOTH, anchor=CENTER, expand=True)
        self.main_pan.pack()
